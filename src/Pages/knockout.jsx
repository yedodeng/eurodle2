...
export default function Knockout() {
  ...
  async function handleFilter(v, key) {
    ...
    if (matchValue(ans, v)) {
      newIncor = [
        ...incor,
        ...people.filter((p) => p[key] !== v).map((p) => p.id),
      ];
    ...
    <div className="space-y-7">
            ...
            <div className="flex flex-wrap gap-1">
              {filters[key].map((v, i) => (
                <button
                  ...
                  disabled={!options.includes(v) && !your.includes(v)}
                  className={`ko-btn 
                                ${
                                  matchValue(ans, v) && chk(key)
                                    ? "success"
                                    : your.includes(v)
                                    ? "error"
                                    : ""
        ...
        {people && (
          <div className="flex flex-wrap gap-2">
            {people.map((v, i) => (
              <button
                key={i}
                onClick={() => handlePerson(v)}
                disabled={incor.includes(v.id)}
                className={`ko-btn ${win && v == ans ? "success" : ""}`}
      ...
      <Modal show={showTut} close={() => setShowTut(false)} btn="X">
        ...
        <div className="m-4 text-xl">
          Each guess, click on a category or person. While clicking on a person
          only knocks out that person, clicking on a category will knockout all
          people in that caetogry. This can also lead to knock on effects as
          other categories get blanked out. Continue doing this until you either
          are left with or knockout the answer.
        </div>
      </Modal>
    </div>
  );
